...
class SS1D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor, z: torch.Tensor):
        self.selective_scan = selective_scan_fn
        ...
        xs = torch.stack([x, torch.flip(x, dims=[-1])], dim=1)  # (b, k, l, d)
        zs = torch.stack([z, torch.flip(z, dims=[-1])], dim=1)  # (b, k, l, d)
        ...
        zs = zs.float().view(B, -1, L)  # (b, k * d, l)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.act(self.conv1d(x))  # (b, d, l)
        x = x.permute(0, 2, 1).contiguous()
        zp = z.permute(0, 2, 1).contiguous()
        y1, y2 = self.forward_core(x,zp)
        assert y1.dtype == torch.float32
        y = torch.cat((y1.unsqueeze(-1), y2.flip(-1).unsqueeze(-1)), dim=-1)
        y = self.conv2d(y.permute(0, 3, 2, 1)).squeeze(1) # (B, L, D)
        y = self.out_norm(y)
        y = y * F.silu(z)
        out = self.out_proj(y)
